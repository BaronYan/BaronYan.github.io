<template><div><p>确保您已经在WSL的Ubuntu中安装了tmux：</p>
<div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="line"><span class="token function">sudo</span> <span class="token function">apt</span> update</span>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><p>在WSL的Ubuntu中编辑您的bash配置文件。通常是 <code v-pre>~/.bashrc</code>,在文件末尾添加以下内容</p>
<div class="language-bash line-numbers-mode" data-highlighter="prismjs" data-ext="sh" data-title="sh"><pre v-pre class="language-bash"><code><span class="line"><span class="token comment"># 检查是否在tmux会话中，以及是否是交互式shell</span></span>
...
<p>重新加载bash配置: <code v-pre>source ~/.bashrc</code></p>
<p>现在，我们需要配置Windows Terminal以在启动时直接进入WSL的Ubuntu。打开Windows Terminal的设置（JSON文件），找到Ubuntu的配置部分，并修改如下：</p>
...
如果仍然没有效果，请检查以下几点：</p>
<ol>
<li>确保tmux已正确安装在WSL的Ubuntu中。
检查 ~/.bashrc 文件中的修改是否正确保存。
确保Windows Terminal的设置正确指向了WSL的Ubuntu。
如果问题仍然存在，可能需要进一步调试。您可以尝试在 ~/.bashrc 文件中添加一些调试输出，例如：</li>
</ol>
<p><code v-pre>echo &quot;Entering custom tmux setup&quot; &gt;&gt; /tmp/debug.log</code></p>
</div></template>
