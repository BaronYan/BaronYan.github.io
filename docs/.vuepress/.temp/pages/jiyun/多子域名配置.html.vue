<template><div><h1 id="多子域名配置" tabindex="-1"><a class="header-anchor" href="#多子域名配置"><span>多子域名配置</span></a></h1>
<ol>
<li>在域名解析设置里设置一条 <code v-pre>CNAME</code> 解析记录: <code v-pre>*.qkcargo.com</code></li>
<li>在服务器管理后端，例如 BT 面板里，设置网站，添加泛域名:<code v-pre>*.qkcargo.com</code></li>
<li>配置 Laravel (在 Laravel 中，你需要设置中间件来处理子域名的重定向。假设每个用户有一个唯一的子域名，你可以在用户登录后进行重定向。)
<ol>
<li><strong>创建中间件</strong>,生成一个新的中间件来处理重定向：<code v-pre>php artisan make:middleware RedirectToSubdomain</code></li>
<li>添加逻辑来处理子域名重定向<div class="language-php line-numbers-mode" data-highlighter="prismjs" data-ext="php" data-title="php"><pre v-pre class="language-php"><code><span class="line"><span class="token keyword">namespace</span> <span class="token package">App<span class="token punctuation">\</span>Http<span class="token punctuation">\</span>Middleware</span><span class="token punctuation">;</span></span>
...
<li>注册中间件</li>
<li>在用户登录后设置子域名,在登录控制器中，设置用户的子域名（假设你在 users 表中有一个 subdomain 列）：<code v-pre>return redirect()-&gt;to(&quot;http://{$subdomain}.yourdomain.com&quot;);</code></li>
</ol>
</li>
</ol>
</div></template>
