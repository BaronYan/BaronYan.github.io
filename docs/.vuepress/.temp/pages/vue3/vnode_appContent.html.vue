...
<li>全局混入（Mixins）：
<ul>
<li>应用级别的混入也存储在 <code v-pre>appContext</code> 中。</li>
</ul>
</li>
<li>插件：
<ul>
<li>通过 <code v-pre>app.use()</code> 安装的插件会修改 <code v-pre>appContext</code>。</li>
</ul>
</li>
<li>全局属性：
<ul>
<li>使用 <code v-pre>app.config.globalProperties</code> 定义的全局属性也是 <code v-pre>appContext</code> 的一部分。</li>
</ul>
</li>
<li>全局配置：
<ul>
...
<li>提供/注入系统：
<ul>
<li><code v-pre>provide</code> 和 <code v-pre>inject</code> API 使用的数据也存储在 <code v-pre>appContext</code> 中。</li>
</ul>
</li>
</ol>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="line">vnode<span class="token punctuation">.</span>appContext <span class="token operator">=</span> appContext<span class="token punctuation">;</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div></div></div><p>这行代码的作用是将传入的 appContext 赋值给新创建的虚拟节点（vnode）。这样做的目的是确保新创建的组件实例能够访问到正确的应用程序上下文。</p>
<p>这在动态创建组件时特别有用，比如在您的消息框组件中。通过设置正确的 <code v-pre>appContext</code>，您确保了：</p>
<ul>
<li>动态创建的组件可以使用全局注册的组件、指令和插件。</li>
<li>可以正确地解析 <code v-pre>provide/inject</code> 关系。</li>
<li>全局配置（如错误处理）可以正确应用到新创建的组件。</li>
</ul>
<p>总之，设置 <code v-pre>appContext</code> 确保了动态创建的组件能够与主应用程序保持一致的上下文环境，使其行为与直接在模板中使用的组件一致。</p>
<p>appContext 的使用主要在以下几个方面：</p>
<ol>
<li>获取 appContext：
<ul>
<li>通常，你可以从根组件或者 app 实例中获取 appContext：</li>
</ul>
</li>
</ol>
...
<li>传递 <code v-pre>appContext</code>：
<ul>
<li>在动态创建组件时，你可以传递 appContext：</li>
</ul>
</li>
</ol>
...
<li>使用 appContext 中的全局资源</li>
</ol>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="line"><span class="token comment">// 在组件内部</span></span>
<span class="line"><span class="token keyword">export</span> <span class="token keyword">default</span> <span class="token punctuation">{</span></span>
<span class="line">  <span class="token function">setup</span><span class="token punctuation">(</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在你的代码中，<code v-pre>appContext</code> 主要用于确保动态创建的消息框组件能够访问到与主应用相同的全局资源和配置。这对于保持一致性和功能完整性非常重要，特别是当你的应用使用了全局组件、指令或者提供了全局数据时。</p>
<p>例如，如果你的消息框组件内部使用了某个全局注册的组件或指令，或者需要访问通过 provide 提供的全局数据，正确设置 <code v-pre>appContext</code> 就变得至关重要。</p>
</div></template>
