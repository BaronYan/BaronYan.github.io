...
<h4 id="创建一个简单的副作用" tabindex="-1"><a class="header-anchor" href="#创建一个简单的副作用"><span>创建一个简单的副作用</span></a></h4>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="line"><span class="token keyword">import</span> <span class="token punctuation">{</span> ref<span class="token punctuation">,</span> watchEffect <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'vue'</span><span class="token punctuation">;</span></span>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个例子中，watchEffect 会自动跟踪 count 的变化，并在 <code v-pre>count.value</code> 改变时执行传入的回调函数。每次 <code v-pre>count.value</code> 发生变化时，控制台都会输出新的值。</p>
<h3 id="特性与优点" tabindex="-1"><a class="header-anchor" href="#特性与优点"><span>特性与优点</span></a></h3>
<ol>
<li><strong>自动依赖追踪</strong>：
<ul>
<li>watchEffect 会自动追踪在其回调函数中使用的所有响应式状态。这意味着你不需要显式地声明要监视的依赖项，Vue 会自动完成这一点。</li>
</ul>
</li>
<li><strong>立即执行</strong>：
<ul>
<li>watchEffect 在创建时会立即执行一次回调函数。这使得你可以在组件挂载时立即执行某些逻辑，而不需要等待状态发生变化。</li>
</ul>
</li>
<li><strong>清理功能</strong>：
<ul>
<li>watchEffect 提供了一个清理机制，允许你在每次副作用重新执行之前执行一些清理操作。例如，在处理异步操作或订阅时，这个功能非常有用。</li>
</ul>
</li>
</ol>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="line"><span class="token keyword">import</span> <span class="token punctuation">{</span> ref<span class="token punctuation">,</span> watchEffect <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'vue'</span><span class="token punctuation">;</span></span>
...
<span class="line"></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个例子中，<code v-pre>onCleanup</code> 函数被用来清除定时器，以防止内存泄漏或其他副作用。</p>
<h3 id="与-watch-的比较" tabindex="-1"><a class="header-anchor" href="#与-watch-的比较"><span>与 watch 的比较</span></a></h3>
<p>虽然 watchEffect 和 watch 都用于响应状态变化，但它们有一些关键的区别：</p>
<ol>
<li>依赖追踪：
<ul>
<li>watchEffect：自动追踪依赖。</li>
<li>watch：需要显式声明依赖。</li>
</ul>
</li>
<li>使用场景：
<ul>
<li>watchEffect：适用于简单的副作用，当依赖关系不明确或不想显式声明时使用。</li>
<li>watch：适用于更复杂的情况，需要对依赖项进行更细粒度的控制。</li>
</ul>
</li>
</ol>
<h4 id="watch-示例" tabindex="-1"><a class="header-anchor" href="#watch-示例"><span>watch 示例</span></a></h4>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="line"><span class="token keyword">import</span> <span class="token punctuation">{</span> ref<span class="token punctuation">,</span> watch <span class="token punctuation">}</span> <span class="token keyword">from</span> <span class="token string">'vue'</span><span class="token punctuation">;</span></span>
<span class="line"></span>
<span class="line"><span class="token keyword">const</span> count <span class="token operator">=</span> <span class="token function">ref</span><span class="token punctuation">(</span><span class="token number">0</span><span class="token punctuation">)</span><span class="token punctuation">;</span></span>
<span class="line"></span>
...
<span class="line">count<span class="token punctuation">.</span>value<span class="token operator">++</span><span class="token punctuation">;</span></span>
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在这个例子中，watch 需要显式地传入要监视的响应式状态 count，并在回调函数中处理新旧值。</p>
<h3 id="结论" tabindex="-1"><a class="header-anchor" href="#结论"><span>结论</span></a></h3>
<p>watchEffect 是一个强大且易用的工具，适用于大多数简单的副作用场景。
当你需要处理更复杂的情况或对依赖项进行精确控制时，可以使用 watch。
通过组合这两种 API，Vue 3 提供了灵活且高效的响应式状态管理解决方案。</p>
</div></template>
