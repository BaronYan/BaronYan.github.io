<template><div><h1 id="import-meta-url-简介" tabindex="-1"><a class="header-anchor" href="#import-meta-url-简介"><span>import.meta.url 简介</span></a></h1>
...
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div></div></div><p>这在现代浏览器中能够原生使用 - 实际上，Vite 并不需要在开发阶段处理这些代码！</p>
<p>这个模式同样还可以通过字符串模板支持动态 URL：</p>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="line"><span class="token keyword">function</span> <span class="token function">getImageUrl</span><span class="token punctuation">(</span><span class="token parameter">name</span><span class="token punctuation">)</span> <span class="token punctuation">{</span></span>
...
<span class="line"></span></code></pre>
<div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>在生产构建时，Vite 才会进行必要的转换保证 URL 在打包和资源哈希后仍指向正确的地址。然而，这个 URL 字符串必须是静态的，这样才好分析。否则代码将被原样保留、因而在 <code v-pre>build.target</code> 不支持 <code v-pre>import.meta.url</code> 时会导致运行时错误。</p>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="line"><span class="token comment">// Vite 不会转换这个</span></span>
...
<p>语法：</p>
<div class="language-javascript line-numbers-mode" data-highlighter="prismjs" data-ext="js" data-title="js"><pre v-pre class="language-javascript"><code><span class="line"><span class="token keyword">new</span> <span class="token class-name">URL</span><span class="token punctuation">(</span>url<span class="token punctuation">)</span></span>
...
<li><code v-pre>url</code><br>
一个表示绝对或相对 URL 的 DOMString 或任何具有字符串化方法的对象，例如 <code v-pre>&lt;a&gt;</code> 或 <code v-pre>&lt;area&gt;</code> 元素。如果<code v-pre> url</code> 是相对 URL，则会将 <code v-pre>base</code> 用作基准 URL。如果 <code v-pre>url</code> 是绝对 URL，则无论参数 <code v-pre>base</code> 是否存在，都将被忽略。</li>
<li><code v-pre>base</code> 可选<br>
一个表示基准 URL 的字符串，当 <code v-pre>url</code> 为相对 URL 时，它才会生效。如果未指定，它默认为 <code v-pre>undefined</code></li>
</ul>
</div></template>
