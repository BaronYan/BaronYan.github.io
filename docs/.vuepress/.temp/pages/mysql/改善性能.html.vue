<template><div><p>MySQL是一个多用户多线程的DBMS，换言之，它经常同时执行多个任务。
如果这些任务中的某一个执行缓慢，则所有请求都会执行缓慢。
如果你遇到显著的性能不良，可使用 <code v-pre>SHOW PROCESSLIST</code> 显示所有活动进程（以及它们的线程ID和执行时间）​。
你还可以用 <code v-pre>KILL</code> 命令终结某个特定的进程（使用这个命令需要作为管理员登录）​。</p>
<ul>
...
<p>客户机软件（用来实际运行MySQL命令的程序）​</p>
<p>几个重要实用程序</p>
<ul>
<li>
<p>MySQL Adiminstrator</p>
</li>
<li>
<p>MySQL Query Browser</p>
</li>
<li>
<p>Navicat</p>
</li>
