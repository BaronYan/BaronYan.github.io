<template><div><h2 id="外键约束" tabindex="-1"><a class="header-anchor" href="#外键约束"><span>外键约束</span></a></h2>
...
<li><strong>定义级联操作</strong>：可以指定当主表中的记录被<strong>更新或删除</strong>时，子表中相 关记录应该如何处理。</li>
</ol>
<p>具体到您的代码：</p>
<ol>
<li><code v-pre>foreign('company_id')</code>：将 <code v-pre>company_id</code> 字段设置为外键。</li>
<li><code v-pre>references('id')</code>：指定这个外键引用的是另一个表的 id 字段。</li>
<li><code v-pre>on('companies')</code>：指定被引用的表是 companies 表。</li>
<li><code v-pre>onDelete('cascade')</code>：当 companies 表中的记录被删除时，相关的 warehouses 记录也会被自动删除。</li>
<li><code v-pre>onUpdate('cascade')</code>：当 companies 表中的 id 更新时，warehouses 表中相应的 company_id 也会自动更新。</li>
</ol>
<p>总的来说，<code v-pre>$table-&gt;foreign()</code> 方法帮助我们在数据库层面建立和维护表之间的关系，确保数据的一致性和完整性。</p>
<h3 id="为空-不为空" tabindex="-1"><a class="header-anchor" href="#为空-不为空"><span>为空|不为空</span></a></h3>
...
<p>是的，&quot;cascade&quot; 是 SQL 语句中的一个关键词，用于定义外键约束的行为。</p>
<p>生变化时，相关联的子表中的记录应该如何处理。
